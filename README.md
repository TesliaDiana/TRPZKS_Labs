...
## Лабораторна робота №2. Контейнеризація застосунку за допомогою Docker Compose

У межах лабораторної роботи проєкт було повністю контейнеризовано з використанням Docker Compose.  
Оркестрація сервісів забезпечує автоматичний запуск, взаємодію та ізоляцію контейнерів у спільному середовищі.

## Архітектура контейнерів

Проєкт складається з декількох взаємопов’язаних сервісів:

- `web` — основний застосунок;
- `db` — база даних PostgreSQL;
- `nginx` — reverse proxy сервер;
- кастомна Docker-мережа для внутрішньої взаємодії контейнерів;
- Docker Volume для збереження даних бази даних.

Усі сервіси працюють в окремій bridge-мережі та взаємодіють між собою через внутрішні DNS-імена Docker.

---

## Попередні вимоги

Перед запуском необхідно переконатися, що у системі встановлено:
...
Для запуску контейнерів потрібно перейти у кореневу директорію проєкту, де знаходиться файл `docker-compose.yml`.
...
Усі сервіси повинні перебувати у статусі `Up`.

---

## 2. Перевірка створення мережі
...
Перевірка контейнерів у кастомній мережі:

```bash
sudo docker network inspect mywebapp_custom-network
```

У результаті відображаються:
...
## 3. Перевірка доступності застосунку

Тестування HTTP-відповіді:
...
Після повторного запуску дані у базі даних залишаються доступними, оскільки Docker Volume не видаляється автоматично.

---

## Аналіз контейнеризації

Використання Docker Compose дозволило:

* автоматизувати запуск усіх сервісів;
* ізолювати компоненти застосунку;
* забезпечити стабільну взаємодію контейнерів через внутрішню мережу;
* спростити перенесення проєкту між різними середовищами;
* забезпечити персистентність даних через Docker Volumes.

Також контейнеризація дозволяє уникнути проблем із локальними залежностями та різницею конфігурацій між середовищами розробки.
